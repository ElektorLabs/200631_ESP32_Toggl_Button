...
bool POWER::setKeepLightLoad(bool en)
{
  // uint8_t data;
  // if (I2C.readByte(IP5306_ADDR, IP5306_REG_SYS_CTL0, &data) == true) {
  //     return I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL0, !en ? (data | LIGHT_LOAD_BIT) : (data & (~LIGHT_LOAD_BIT)));
  // }
  return false;
}
...
bool POWER::setLowPowerShutdown(bool en)
{
  //uint8_t data;
  //if (I2C.readByte(IP5306_ADDR, IP5306_REG_SYS_CTL1, &data) == true) {
  //  return I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL1, en ? (data | LOWPOWER_SHUTDOWN_BIT) : (data & (~LOWPOWER_SHUTDOWN_BIT)));
  //}
...
void POWER::powerOFF(){
  ...
  */
  //Power off request
  if (I2C.readByte(IP5306_ADDR, IP5306_REG_SYS_CTL1, &data) == true){
    I2C.writeByte(IP5306_ADDR, IP5306_REG_SYS_CTL1, (data & (~BOOST_ENABLE_BIT)));
  }
