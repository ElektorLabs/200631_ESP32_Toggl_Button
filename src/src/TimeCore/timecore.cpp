 ...
 *    Function      : begin
 *    Class         : Timecore
 *    Description   : gets timcounting going
 *    Input         : bool use_ticker
 *    Output        : none
 *    Remarks       : can optinal use a ticker for 1s pulses
 **************************************************************************************************/
void Timecore::begin( bool use_ticker  ){
  if(true == use_ticker ){
...
**************************************************************************************************/  
void Timecore::RTC_Tick( Timecore* pThis ){ /* Needs to be called once a second */
    pThis->local_softrtc_timestamp++;
    if(pThis->DegradeTimer_Src>0){
     pThis->DegradeTimer_Src--;
     
    } else {
      if(pThis->CurrentMasterSource>NO_RTC){
        pThis->CurrentMasterSource= (source_t)((int)(pThis->CurrentMasterSource-1));
      }
      pThis->DegradeTimer_Src=900;
    }
}    
...
time_t Timecore::GetLocalTime( void )
{
  ...
  time_t converted = ConvertToLocalTime( now );
  return converted;
}
 ...
 /**************************************************************************************************
     ...
     *    Class         : Timecore
     *    Description   : Gets the local time from UTC stamp
     *    Input         : uint32_t 
     *    Output        : time_t
     *    Remarks       : Returns the local time according to the settings
     **************************************************************************************************/

time_t Timecore::ConvertToLocalTime( time_t now ){
